...
| CLI Flag | Env Var | Config File | Default | Description |
| -------- | ------- | ----------- | ------- | ----------- |
| `--mesh.direct-peers` | `MESH_DIRECT_PEERS` | `mesh.direct-peers` | `[]` | Peers to request direct edges to. |
...
| `--bootstrap.advertise-address` | `BOOTSTRAP_ADVERTISE_ADDRESS` | `bootstrap.advertise-address` |  | Address to advertise for raft consensus |
...
| `--bootstrap.listen-address` | `BOOTSTRAP_LISTEN_ADDRESS` | `bootstrap.listen-address` | `[::]:9001` | Address to use when using TCP raft consensus to bootstrap |
...
| `--bootstrap.servers` | `BOOTSTRAP_SERVERS` | `bootstrap.servers` | `[]` | Map of node IDs to raft addresses to bootstrap with |
| `--bootstrap.servers-grpc-ports` | `BOOTSTRAP_SERVERS_GRPC_PORTS` | `bootstrap.servers-grpc-ports` | `[]` | Map of node IDs to gRPC ports to bootstrap with |
...
| CLI Flag | Env Var | Config File | Default | Description |
| -------- | ------- | ----------- | ------- | ----------- |
| `--raft.apply-timeout` | `RAFT_APPLY_TIMEOUT` | `raft.apply-timeout` | `10s` | Raft apply timeout. |
| `--raft.commit-timeout` | `RAFT_COMMIT_TIMEOUT` | `raft.commit-timeout` | `10s` | Raft commit timeout. |
| `--raft.connection-pool-count` | `RAFT_CONNECTION_POOL_COUNT` | `raft.connection-pool-count` | `0` | Raft connection pool count. |
| `--raft.connection-timeout` | `RAFT_CONNECTION_TIMEOUT` | `raft.connection-timeout` | `5s` | Raft connection timeout. |
| `--raft.data-dir` | `RAFT_DATA_DIR` | `raft.data-dir` | `/var/lib/webmesh/store` | Raft data directory. |
| `--raft.election-timeout` | `RAFT_ELECTION_TIMEOUT` | `raft.election-timeout` | `1s` | Raft election timeout. |
| `--raft.heartbeat-purge-threshold` | `RAFT_HEARTBEAT_PURGE_THRESHOLD` | `raft.heartbeat-purge-threshold` | `25` | Raft heartbeat purge threshold. |
| `--raft.heartbeat-timeout` | `RAFT_HEARTBEAT_TIMEOUT` | `raft.heartbeat-timeout` | `1s` | Raft heartbeat timeout. |
| `--raft.in-memory` | `RAFT_IN_MEMORY` | `raft.in-memory` | `false` | Run raft with in-memory storage. |
| `--raft.leader-lease-timeout` | `RAFT_LEADER_LEASE_TIMEOUT` | `raft.leader-lease-timeout` | `1s` | Raft leader lease timeout. |
| `--raft.listen-address` | `RAFT_LISTEN_ADDRESS` | `raft.listen-address` | `[::]:9000` | Raft listen address. |
| `--raft.log-level` | `RAFT_LOG_LEVEL` | `raft.log-level` | `info` | Raft log level. |
| `--raft.max-append-entries` | `RAFT_MAX_APPEND_ENTRIES` | `raft.max-append-entries` | `64` | Raft max append entries. |
| `--raft.observer-chan-buffer` | `RAFT_OBSERVER_CHAN_BUFFER` | `raft.observer-chan-buffer` | `100` | Raft observer channel buffer. |
| `--raft.prefer-ipv6` | `RAFT_PREFER_IPV6` | `raft.prefer-ipv6` | `false` | Prefer IPv6 connections for the raft transport. |
| `--raft.request-observer` | `RAFT_REQUEST_OBSERVER` | `raft.request-observer` | `false` | Request to be an observer in raft. |
| `--raft.request-vote` | `RAFT_REQUEST_VOTE` | `raft.request-vote` | `false` | Request a vote in raft elections. |
| `--raft.snapshot-interval` | `RAFT_SNAPSHOT_INTERVAL` | `raft.snapshot-interval` | `30s` | Raft snapshot interval. |
| `--raft.snapshot-retention` | `RAFT_SNAPSHOT_RETENTION` | `raft.snapshot-retention` | `2` | Raft snapshot retention. |
| `--raft.snapshot-threshold` | `RAFT_SNAPSHOT_THRESHOLD` | `raft.snapshot-threshold` | `8192` | Raft snapshot threshold. |
...
| `--wireguard.masquerade` | `WIREGUARD_MASQUERADE` | `wireguard.masquerade` | `true` | Enable masquerading of traffic from the wireguard interface. |
...
| `--discovery.kad-bootstrap-servers` | `DISCOVERY_KAD_BOOTSTRAP_SERVERS` | `discovery.kad-bootstrap-servers` | `[]` | list of bootstrap servers to use for the DHT |
...
| `--services.api.stun-servers` | `SERVICES_API_STUN_SERVERS` | `services.api.stun-servers` | `[stun:stun.l.google.com:19302]` | STUN servers to use for the WebRTC API. |
| `--services.api.webrtc-enabled` | `SERVICES_API_WEBRTC_ENABLED` | `services.api.webrtc-enabled` | `false` | Enable and register the WebRTC API. |
| `--services.disable-leader-proxy` | `SERVICES_DISABLE_LEADER_PROXY` | `services.disable-leader-proxy` | `false` | Disable the leader proxy. |
| `--services.grpc-listen-address` | `SERVICES_GRPC_LISTEN_ADDRESS` | `services.grpc-listen-address` | `[::]:8443` | gRPC listen address. |
| `--services.grpc-web-enabled` | `SERVICES_GRPC_WEB_ENABLED` | `services.grpc-web-enabled` | `false` | Enable gRPC over HTTP/1.1. |
| `--services.insecure` | `SERVICES_INSECURE` | `services.insecure` | `false` | Disable TLS. |
| `--services.meshdns.cache-size` | `SERVICES_MESHDNS_CACHE_SIZE` | `services.meshdns.cache-size` | `0` | Size of the remote DNS cache (0 = disabled). |
...
| `--services.tls-cert-data` | `SERVICES_TLS_CERT_DATA` | `services.tls-cert-data` |  | TLS certificate data. |
| `--services.tls-cert-file` | `SERVICES_TLS_CERT_FILE` | `services.tls-cert-file` |  | TLS certificate file. |
| `--services.tls-key-data` | `SERVICES_TLS_KEY_DATA` | `services.tls-key-data` |  | TLS key data. |
| `--services.tls-key-file` | `SERVICES_TLS_KEY_FILE` | `services.tls-key-file` |  | TLS key file. |
...
| `--plugins.debug.listen-address` | `PLUGINS_DEBUG_LISTEN_ADDRESS` | `plugins.debug.listen-address` | `localhost:6060` | Address to listen on |
...
| `--plugins.debug.pprof-profiles` | `PLUGINS_DEBUG_PPROF_PROFILES` | `plugins.debug.pprof-profiles` | `[]` | Pprof profiles to enable |
| `--plugins.ipam.static-ipv4` | `PLUGINS_IPAM_STATIC_IPV4` | `plugins.ipam.static-ipv4` | `[]` | Static IPv4 assignments. |
